...
class FSSD(nn.Module):
    ...
    def __init__(self, base, extras, ft_module, pyramid_ext, head, num_classes, size):
        ...
        self.upsample0 = nn.Upsample(size=(3, 3), mode='bilinear',align_corners=True)

        self.conv_cat1 = nn.Conv2d(512, 256, kernel_size=1, padding=0, stride=1)
        self.upsample1 = nn.Upsample(size=(5, 5), mode='bilinear',align_corners=True)

        self.conv_cat2 = nn.Conv2d(512, 256, kernel_size=1, padding=0, stride=1)
        self.upsample2 = nn.Upsample(size=(10, 10), mode='bilinear',align_corners=True)

        self.conv_cat3 = nn.Conv2d(768, 512, kernel_size=1, padding=0, stride=1)
        self.upsample3 = nn.Upsample(size=(19, 19), mode='bilinear',align_corners=True)

        self.conv_cat4 = nn.Conv2d(1024, 512, kernel_size=1, padding=0, stride=1)
        self.upsample4 = nn.Upsample(size=(38, 38), mode='bilinear',align_corners=True)
